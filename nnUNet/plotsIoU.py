...
file_path = r"C:\Users\PolDiaz\Desktop\nnUNet_test\Results\IoU_global.xlsx"
output_dir = r"C:\Users\PolDiaz\Documents\GitHub\segmentation_ai_v2\nnUNet\results_iou_plots"
os.makedirs(output_dir, exist_ok=True)

# Loading data
df_labels = pd.read_excel(file_path, sheet_name="IoU_perLabel", header=1)
df_regions = pd.read_excel(file_path, sheet_name="IoU_perRegion", header=1)

df_labels["IoU_Diff"] = df_labels["Mean_IoU_Fine"] - df_labels["Mean_IoU_Orig"]
df_regions["IoU_Diff"] = df_regions["Mean_IoU_Fine"] - df_regions["Mean_IoU_Orig"]
...
# Plot difference IoU per vertebra
...
plt.title("Difference in IoU (Fine-tuned - Original) per vertebra")
plt.xlabel("Vertebra")
plt.ylabel("IoU Difference")
plt.grid(True, axis="y", alpha=0.3)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "IoU_diff_by_label.png"), dpi=300)
plt.close()
...
orig_data = [df_labels[df_labels["Region"]==r]["Mean_IoU_Orig"] for r in regions]
fine_data = [df_labels[df_labels["Region"]==r]["Mean_IoU_Fine"] for r in regions]
...
plt.title("Comparison of IoU across vertebral regions (Original vs Fine-tuned)")
plt.ylabel("IoU")
plt.grid(True, axis="y", alpha=0.3)
...
# Plot IoU Fine-tuned model with sample sizes
fig, ax1 = plt.subplots(figsize=(12,6))
ax1.plot(
    df_labels["Label_Name"],
    df_labels["Mean_IoU_Fine"],
    marker="o",
    linewidth=2,
    color="#b81414",
    label="Mean IoU"
...
plt.title("Mean IoU (fine-tuned) per vertebra with sample size")
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "IoU_fine_by_label_with_Nsamples.png"), dpi=300)
plt.close()
